...
function azs_registration
{
  ...
  local FILENAME=deploy-$(echo $ARGUMENTS_JSON | jq -r ".tenantSubscriptionId")-$(date --utc +y%Ym%md%dh%Hm%M)-v$(sudo cat /azs/common/config.json | jq -r ".version.script").log
